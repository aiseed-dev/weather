...
**状態: v1 実装済み(2026-07-07、fetch_tiles.py)。** z2 モザイクの目視検証済み
(7 月の北半球でサハラ〜中東 40℃超・南極 −40℃帯・グリーンランド/アンデスの低温が正しく描画)。

ECMWF オープンデータ(IFS、0.25°格子、CC BY 4.0)から世界の天気の**ラスタータイル**を生成し、
weather.time-j.net の新しい世界天気ページと、time-j.net の世界地図(/WorldTime/Map)のレイヤーとして配信する。

### 方式(v1 実装)

- **要素(v1)**: 2m気温(`t2m`)。v2 以降で 降水量・海面更正気圧(等圧線)・風。
- **タイル形式**: XYZ 256px PNG、z0〜4(世界表示用)。全ズーム合計 341 タイル/要素/時刻。
- **時刻**: 解析 0h + 予報 24/48/72h(06/18Z ランも 90h まで公開されるため 72h まで取れる)。
- **パイプライン**: fetch_tiles.py(ECMWF open data の .index で 2t だけ HTTP Range 取得、
  約 0.5MB/時刻)→ cfgrib デコード → LUT で色塗り → 最近傍サンプリングでタイル分割
  → `public/tiles/{element}/{step}h/{z}/{x}/{y}.png` + `public/tiles/latest.json`(ラン・凡例)。
- **依存**: weather リポジトリの conda 環境 `../.venv`(cfgrib/eccodes 導入済み)をそのまま使用。
  専用環境は不要だった。cron は docs/operations.md(JST 17:30/23:30/5:30/11:30 の 4 回)。
- **配信**: Cloudflare Pages。タイルは毎回総入替、1要素×4時刻×341タイル ≒ 1,400ファイル/回の
  デプロイ増(Pages 上限 20,000 に対しては問題なし)。
- **出典表記**: 「Data: ECMWF open data (CC BY 4.0)」を latest.json の `attribution` で配信し、
  ページ側に明記。

### 決定済み(2026-07-07)

- **モデル: IFS**。v2 予定の降水・海面更正気圧・風まで全要素が揃い、144h まで 3 時間刻み。
  AIFS は要素・時間刻みに制約があるため不採用。
- **更新頻度: 1 日 4 回(00/06/12/18Z 全ラン)**。
- **出力形式: Web Mercator XYZ タイルのみ**(ユーザー決定「タイルなら地図を動かせる」)。
  正距円筒 PNG の併産は見送り。地図表示はパン・ズーム可能な Leaflet 系を前提とする。
  帰結: /WorldTime/Map の正距円筒 SVG にはこのタイルを直接重ねられない。
  Map 側を Leaflet(Mercator)化するか、必要になった時点で正距円筒 PNG を追加する
  (同じ色配列からの書き出しなので後付けは安価)。
- ズーム範囲は計画どおり z0〜4。データが 0.25° 格子なので z3 でほぼ等倍、
  それ以上はクライアント側のオーバーズーム(Leaflet の maxNativeZoom)で補間拡大すれば足りる。
- **色スケール: 気象で一般的な青→赤の連続スケール**(−40〜+45℃、極寒側は紫)。
  /WorldTime/Map の点表示とは統一しない。**fetch_tiles.py の `ANCHORS` が配色の正典**で、
  latest.json の `legend` として配信する(/WorldTime/Map が点表示に採用する場合もここを参照)。

未決事項はなし(全項目決定済み)。

## 運用構想: クラウド生成と過去データ基盤(2026-07-08 ブレスト、未着手)

タイル生成と ERA5 過去データ処理をクラウドへ移し、Cloudflare で配信する構想。
AWS 契約・Cloudflare 設定・公開などの外部操作はすべてユーザー自身が行う。

### 計算: 2段構え

| 用途 | 形態 | リージョン | 費用目安 |
|------|------|-----------|---------|
| 毎日のタイル生成(1日4回) | 常設の最小インスタンス(t4g.nano 等) | **eu-central-1**(`ecmwf-forecasts` と同居) | 月$4〜5 |
| ERA5 の大バッチ(平年値等、一度きり) | 使い捨てスポット(c7g 等、成果物を上げたら破棄) | **us-east-1**(`ecmwf-era5` と同居) | 1回数ドル |

- 同リージョンなら S3 読み出しは無料・高速。150MB/ファイルの GRIB 丸ごとや
  ERA5 30年分(1変数 約1TB)の読み捨て集計が現実的になる。
- v1 の Range 取得は 1日8MB なので自宅 cron でも困らない。移行の動機は
  「常時稼働の自動化」と v2 以降(要素追加・3時間刻みアニメーション・ERA5)のスケール。

### 配信: R2 と Pages の役割分担

- **HTML/CSS/JS(低頻度更新)→ Pages**(従来どおり)。
- **タイル・データ成果物(高頻度・大容量)→ R2**。理由:
  - Pages の 25MB/ファイル制限(ERA5 成果物が当たる)と、direct upload が毎回
    全ファイルのハッシュ照合を行う無駄(データ更新のたびにサイト全体を照合)。
  - なお Pages の「500/月」制限は git 連携 CI ビルドの回数。cf-publish の
    direct upload は消費しない(2026-07-08 ドキュメント確認済み)。
  - R2 は egress 無料・書込は無料枠100万回/月(タイルは約17万回/月)。
    ストレージ無料枠 10GB 超過は自動従量課金($0.015/GB・月)で、停止はしない。
- URL: R2 に独自ドメイン(例 tiles.weather.time-j.net)を付け、latest.json の
  `tile_url` を絶対URLにする(消費側の変更はそれだけ)。CORS はバケット側で設定。
- キャッシュ: 同一URL上書きになるため Cache-Control を明示(タイルは max-age 30分程度)。
  きれいにやるならパスにラン時刻を入れ、latest.json だけ短TTL+古いランは
  ライフサイクルルールで自動削除。
- 上げ口は cf-publish 0.2.0 の r2 sync。

### 過去データは2層で扱う

| 層 | 中身 | サイズ | 置き方 |
|----|------|--------|--------|
| **集計値**(固定) | 平年値 1991〜2020(WMO標準)、都市別気候JSON | 数GB(1変数の日別グリッド約1.5GB) | R2 標準。一度計算すれば固定資産 |
| **アーカイブ**(成長) | 過去の日々のタイル(1940年〜) | 約150GB/要素、3要素で約0.5TB、刻み/ズーム増で数TB | 下記3策 |

アーカイブ側の設計(効く順):

1. **遅延生成**: 全期間の事前生成はせず、リクエストされた日を初回だけ生成して
   R2 に永久キャッシュ。アーカイブは実際に見られた分しか増えない(最大の削減策)。
2. **階層化**: 直近は R2 標準、古いものは R2 Infrequent Access($10/TB・月、
   無料枠対象外・取得課金あり)や Backblaze B2($6/TB・月、Cloudflare 経由
   egress 無料提携)。
3. **協力者シャード**(下記)。

ERA5 解禁で作れるもの: 平年差(アノマリー)タイル(v2 本命候補)、世界版雨温図
(worldtime 532都市に平年気候を付与)、世界の気温ランキング・歴史的事例の再現図。

### 協力体制(将来、基盤が一人で回るようになってから)

- **正典は自分の R2**。協力者のストレージは複製であり、サイトが読む先にはしない
  (協力者の離脱・設定ミスを自サイトの障害にしない)。
- 小規模なうちは**ミラー**(冗長化目的): R2 は egress 無料なので同期コストゼロ。
  rclone のコピペ2行で済む自己完結の説明書を用意する(非エンジニア協力者前提)。
- アーカイブが TB 級になったら**シャード分担**(期間・要素で分割、
  例「1940〜60年代はAさんのバケット」)。正典側の manifest.json が日付→ホストを引く。
- 検証: `SHA256SUMS` を成果物と併置し、ミラー/シャードの改竄・破損を機械的に検証可能にする。
- 成立根拠: 成果物は全て公開データ(ERA5、CC BY 4.0)からの**決定論的な再計算物**。
  シャード消失は「データ喪失」ではなく「再計算の手間」であり、有志分散が許容できる。
- ライセンスは整合済み: コード AGPL-3.0、データ CC BY 4.0(出典表記で再配布可)。
  公開する場合は成果物仕様を DATA_CONTRACT 方式で明文化してから。

### 順序

1. 常設 EU インスタンス + R2 移行(タイル)を一人で回る形にする
2. スポットで平年値バッチ → 集計値層を R2 へ
3. v2 プロダクト(アノマリータイル等)
4. 公開・協力者募集(ミラー → 必要になればシャード)
