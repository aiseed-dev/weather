...
**ただし AI コーディング前提なら、これは実質的に問題にならない** ―
後述の Skill 機能 (section 10) で「うちは Flet 0.85+ 宣言的モード
のみ、`page.update()` 禁止」と一度書いておけば、AI は古いチュート
リアルを参考にせず、最新 API で生成する。`aiseed-weather` でも
`flet-declarative` Skill にこの規約を書いてあるので、Web 検索ベース
で混入する古い書き方を AI が選ぶことは無い。
