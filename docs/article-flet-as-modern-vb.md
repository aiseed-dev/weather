...
### Python のコードが書けなくても大丈夫

ここで強調しておきたいのは、**Python の文法を知っている必要は無い**
こと。必要なのは:

- 作りたいものを**言葉で明確に説明できる**こと
- 出てきた画面を見て**「こうじゃない」「これは合ってる」と判断
  できる**こと
- バグや違和感を**言葉で伝えられる**こと

たったこれだけ。コードを読み書きする能力は不要。**「要件を言語化
できる人」がそのまま「アプリを作れる人」になる**、というのが
2026 年現在の業務開発の到達点。

### 構造としては SIer 発注と同じ

この体験は、社外の SIer (システムインテグレータ) に開発を発注する
プロセスとほぼ同じ構造をしている:

| | SIer に発注 | Claude (AI) に発注 |
|---|---|---|
| 何を作るか伝える | 要件定義書 / 仕様書 | チャット + 計画書 (`docs/plan.md`) |
| 規約を共有する | 開発標準書 / コーディング規約 | Skill (`.agents/skills/`) |
| 実装してもらう | 数週〜数ヶ月 | 数分〜数時間 |
| 動作確認 / フィードバック | UAT / バグチケット | 動かしてチャットで指摘 |
| 仕様変更 | 変更管理 / 追加見積 | チャットで言うだけ |
| 単価 | 人月単価 (数十万〜数百万) | サブスク / API 従量課金 |
| 24/365 対応 | 不可 | 可 |
| レビューは人間がやる | ◎ | ◎ |

**「自分でコードを書く能力ではなく、要件を整理して人に伝える能力」
が業務遂行の中心になる**点では、SIer 発注を経験したことがある人なら
そのまま入れる。違いは:

- **サイクルが圧倒的に速い** ― 「こうしたい」と言ってから動くものが
  出るまでの時間が、人月単位 → 分単位に
- **コストが桁違いに低い** ― SIer なら見積もり→稟議→契約の数週間が、
  サブスク契約 1 つで消える
- **小さな試行錯誤を躊躇しなくなる** ― 「ちょっと UI 変えてみよう」が
  数十秒で試せるので、業務側の判断が速くなる

SIer に「アプリを作ってもらう」発想がある現場なら、その発注先を
社内の Claude に置き換えるだけ、というのが直感的な理解。**コードを
書ける人を雇うのではなく、要件を語れる人がツールを持つ**時代に
なっている。
